...
import com.way.common.util.SendMsgAsyncTask.OnSendSuccessListener;
...
public class FirstSetActivity extends Activity implements OnClickListener,
	...
	private void initData() {
		...
		mSpUtil = mApplication.getSpUtil();	// 配置参数
	...
	public void onClick(View v) {
		...
		case R.id.first_start_btn:
			...
			mSpUtil.setTag(SexAdapter.SEXS[mSexWheel.getCurrentItem()]); // 按性别 set tag?
	...
	public void onBind(String method, int errorCode, String content) {
		if (errorCode == 0) {// 如果绑定账号成功，由于第一次运行，给同一tag的人推送一条新人消息
			User u = new User(mSpUtil.getUserId(), mSpUtil.getChannelId(),
					mSpUtil.getNick(), mSpUtil.getHeadIcon(), 0);	// group: 0
			mUserDB.addUser(u);// 把自己添加到数据库
			
			task = new SendMsgAsyncTask(
					JsonUtil.createJsonMsg(System.currentTimeMillis(), "hi", mSpUtil.getTag()),
					""	//发给所有人
					);
			task.setOnSendSuccessListener(new OnSendSuccessListener() {
	...
	public void onNotify(String title, String content) {
		L.d("TODO: FirstSetActivity.onNotify");
	}
