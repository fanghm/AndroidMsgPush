...
// TODO: need refactor it to send REST msg to specified server (app server, or Baidu push server)
public class BaiduPush {
	// server type
	public final static int PUSH_SERVER = 0;
	public final static int APP_SERVER = 1;
	
	public final static String mPushServerUrl = "http://channel.api.duapp.com/rest/2.0/channel/";	// PushServer url
	public final static String mAppServerUrl = "http://127.0.0.1:3000/api/apiv1/all_user/?apikey=";
	 ...
	 *            应用key
	 */
	public BaiduPush(int server, String http_mehtod, String secret_key, String api_key) {
		mUrl = getServer(server);
		mHttpMethod = http_mehtod;
		mSecretKey = secret_key;
		mApiKey = api_key;
	}

	private String getServer(int server) {
		if (APP_SERVER == server)
			return mAppServerUrl;
		else
	...
	public String HttpGetRequest() {
		...
		String url = mAppServerUrl + mApiKey;
	...
	public String PushMessage(String message) {
		...
		ra.put(RestApi._MESSAGE_KEYS, MSGKEY);
		// ra.put(RestApi._MESSAGE_EXPIRES, "86400");
		ra.put(RestApi._PUSH_TYPE, RestApi.PUSH_TYPE_ALL); // push_type = 3
		// ra.put(RestApi._DEVICE_TYPE, RestApi.DEVICE_TYPE_ANDROID);
		return PostHttpRequest(ra);
	}
